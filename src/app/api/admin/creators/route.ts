...
const creatorSchema = z.object({
  ...
  totalLikes: z.coerce.number().int().min(0).default(0).optional(),
...
export async function POST(req: NextRequest) {
  ...
  const creator = await prisma.creator.create({
    data: {
      ...
      platformStats: (data.platformStats as unknown as never) ?? undefined,
      followerCount: data.followerCount,
      views: data.views,
      totalLikes: data.totalLikes ?? 0,
      lastScrapedAt: data.platformStats ? new Date() : null,
